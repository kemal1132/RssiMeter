...
public class ScanActivity extends AppCompatActivity {
    ...
    private static final int OBSERVATIONTIMESECONDS = 5;
    ...
    public void saveOnCSV(String[] data){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        { ActivityCompat.requestPermissions(ScanActivity.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, 10 );}
        Log.d(TAG, "Permission granted I hope" );
            try {
                File file = new File(filepath);
                if (file.exists() && !file.isDirectory()) {
                    FileWriter aFileWriter = new FileWriter(file, true);
                    writer = new CSVWriter(aFileWriter);
                } else {
                    ...
                    new Thread(new Runnable() {
                        public void run() {
                           Log.d(TAG, "Button clicked ------------------------------------------------------------" );
                            for(int timeCounter = 0; timeCounter<OBSERVATIONTIMESECONDS; timeCounter++){
                                String locationText = ((TextView) findViewById(R.id.saveLocation)).getText().toString();
                                String deviceText = ScanActivity.this.getScannedDevices().get(0);


                                  for (int i = 0; i < ScanActivity.this.getScannedDevices().size(); i++) {
                                     String[] data = ScanActivity.this.getScannedDevices().get(i).split("\n");
                                        if (data[1].equals("null")) {
                                            //Toast.makeText(ScanActivity.this,"-"+data[1]+"-", Toast.LENGTH_SHORT).show();;
                                            data[1] = data[0];
                                            data[0] = locationText;

                                            String[] dataWithTime = new String[data.length+1];
                                            System.arraycopy(data, 0, dataWithTime, 0, data.length);
                                            Date date = new Date();

                                            dataWithTime[data.length]=date.toString();

                                            ScanActivity.this.saveOnCSV(dataWithTime);//this saves the data to csv
                                            Log.d(TAG, "Succesfully written device information on csv for" + locationText);
    ...
    private ScanCallback mScanCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            Log.i(TAG, result.getDevice().getAddress()+" - RSSI: "+result.getRssi()+"\t - "+" - "+result.getRssi()+"\n"+result.getDevice().getName()+ "\n"+result.getDevice().getAddress()+"\n");
            int timesSeen = 0;
            ...
            for(int i=0; i<scannedDeivcesList.size(); i++){
                ...
                //added this part to keep track of how many times rssi is observed

                String[] data = scannedDeivcesList.get(i).split("\n");
                if(data.length<4){
                    timesSeen=0;
                }else if(data.length>0){
                    timesSeen = Integer.parseInt(data[3]);
                }
                else{
                    timesSeen=0;
                }
                ...
                if(scannedDeivcesList.get(i).contains(result.getDevice().getAddress())){
                    ...
                    timesSeen++;
                    scannedDeivcesList.set(i, result.getRssi()+"\n"+result.getDevice().getName()+ "\n"+result.getDevice().getAddress()+"\n"+timesSeen);
                    break;
                }
            }
            ...
            if(!contains){
                ...
                scannedDeivcesList.add(result.getRssi()+"\n"+result.getDevice().getName()+ "\n"+result.getDevice().getAddress()+"\n"+timesSeen);
            }
